...
def initController():
    ...
    def post():
        ...
        nfs_control = nfs.split("-")[0]
        nfs_data = nfs.split("-")[1]
        nfs_list.append(nfs)
        ...
        if num_nsfs == 1:
            vpn_servers.append(nfs_data)
            configureServer(nfs_control,nfs_data)
            return 'OK'

        # Add clients to VPN #

        if num_nsfs > 1:
            createSA(nfs_control,nfs_data)
            return 'OK'
...
def configureServer(nfs_control, nfs_data):
    ...
    # load cert #
    fd = open("/tmp/cert."+nfs_control+".pem","r")
    nfs_cert = fd.read()
...
def createSA(client_control, client_data):
    ...
    client_conf = client_conf.replace("SA-ID","1")
    client_conf = client_conf.replace("SERVER-ADDRESS",server)
    client_conf = client_conf.replace("LAST-NSF-FLAG","true")
    client_conf = client_conf.replace("CA-CERTIFICATE",ca_cert)
    client_conf = client_conf.replace("CLIENT-CERT",nfs_cert)
    client_configuration = client_configuration + client_conf
    client_configuration = client_configuration + "</config>"

    m = manager.connect_ssh(host=client_control, port=830, timeout=None, username="javier", password=None, key_filename=None, allow_agent=True, hostkey_verify=True, look_for_keys=True, ssh_config=None)
    m.async_mode = True
    m.edit_config(target='running', config=client_configuration, test_option='test-then-set')
    configuration.close()
